...
from .config import AppConfig, PAPER_TYPE_LABELS
from .extract import collect_papers, extract_paper
from .grader import DimensionResult, GradeResult, Grader
...
def grade_files(args) -> int:
    ...
    print(f"共发现 {len(files)} 篇论文，类型策略：{args.type}"
          + ("，mock 试运行" if args.mock else ""))
    client = LLMClient(cfg.llm)
    if not args.mock and not client.enabled:
        print("\n[提示] 未配置 API Key：请 export PAPER_GRADER_API_KEY=... "
              "或使用 --mock 模式试跑。\n")
        return 1
    ...
    def work(item: tuple[Path, str]) -> GradeResult:
        ...
        try:
            ...
            res = GradeResult(
                file=path.name, title=path.stem, ptype=ptype,
                rubric_name=rubric.name, total=0, band="—",
                confidence=0, dimensions=[], error=f"{type(e).__name__}: {e}"[:300],
            )
        return res
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for fut in as_completed(futs):
                ...
                if res.error:
                    ...
                    print(f"  [{done}/{len(pending)}] {res.file} → {res.total} 分"
    ...
    for r in sorted(ok, key=lambda x: -x.total):
        report_path = save_report(r, out_dir / "报告", model_note=f"AI 辅助（{cfg.llm.model}）" if not r.mock else "mock 试运行")
